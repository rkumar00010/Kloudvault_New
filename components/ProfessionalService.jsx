...
const HERO_PATHS = ["/ProfessionalServices-Hero.png", "/ProfessionalServices-Hero.jpg"];
const ENHANCE_IMG_PATHS = ["/Enhance.png", "/Enhance.jpg", "/enhance.png"];
const RESOLVE_IMG_PATHS = ["/Resolve.png", "/Resolve.jpg", "/resolve.png"];

const statCards = [
  { big: "24x7", small: "Client Support Readiness" },
  { big: "Zero", small: "Escalation Delays" },
  { big: "99.9%", small: "Service Reliability" },
  { big: "Smart", small: "Expert Routing" },
  { big: "Salesforce-Native", small: "Service Workflows" },
  { big: "Enterprise", small: "Data Governance" },
];

function statCardClass(i) {
  return i % 2 === 0
    ? "border border-slate-300/50 bg-[#D1E3F2] shadow-sm"
    : "border border-slate-400/40 bg-[#A6CCE1] shadow-sm";
}

export default function ProfessionalService() {
  const [heroBgIndex, setHeroBgIndex] = useState(0);
  const heroBgFailed = heroBgIndex >= HERO_PATHS.length;
  const [enhanceImgIndex, setEnhanceImgIndex] = useState(0);
  const [resolveImgIndex, setResolveImgIndex] = useState(0);

  const impactStats = [
    {
      pct: "34%",
      title: "Faster Case Resolution",
      body: "Intelligent routing and seamless transfers help service teams resolve client issues quickly across multi-step workflows.",
    },
    {
      pct: "38%",
      title: "Improved Team Productivity",
      body: "Automated follow-ups and unified call records reduce repetitive admin work so consultants focus on higher-value client outcomes.",
    },
    {
      pct: "31%",
      title: "Higher Client Retention",
      body: "Consistent communication quality and faster support cycles strengthen trust, improve renewal confidence, and reduce relationship churn.",
    },
  ];

  const enhanceCards = [
    {
      title: "Deliver Context-Aware Client Conversations",
      body: "Give consultants full account context before each call to improve clarity, reduce repetition, and personalize every interaction.",
    },
    {
      title: "Automate Follow-Ups and Status Updates",
      body: "Use workflow-driven call tasks and reminders so clients receive timely updates while teams maintain consistency across service commitments.",
    },
    {
      title: "Route Clients to the Right Specialist",
      body: "Direct calls by expertise, priority, and service line to reduce handoffs and help clients reach the right expert from the first interaction.",
    },
  ];

  const resolveCards = [
    {
      title: "Resolve Escalations with Better Collaboration",
      body: "Enable warm transfers and real-time conference support so teams can solve complex issues faster without communication breakdowns.",
    },
    {
      title: "Streamline Appointment Coordination",
      body: "Automate scheduling and reminders to reduce no-shows, minimize back-and-forth, and improve consultant calendar efficiency.",
    },
    {
      title: "Coach Teams with Conversation Insights",
      body: "Use call monitoring and outcome reviews to improve service quality, advisor performance, and consistency across client-facing teams.",
    },
  ];

  const testimonials = [
    {
      title: "Clear improvement in service operations",
      quote:
        "Our teams now manage client communication in one Salesforce flow. Escalations are faster to resolve, and follow-ups are far more structured than before.",
      name: "Nikhil Arora",
      role: "Client Services Director",
    },
    {
      title: "Reliable communication across distributed teams",
      quote:
        "Consultants and support teams stay aligned with real-time call visibility and better workflow discipline. It helped us improve client confidence and responsiveness.",
      name: "Sanya Bhargava",
  ...
  return (
    <div id="top" className="min-h-screen bg-white text-slate-900">
        ...
        <div className="relative z-10 mx-auto flex w-full max-w-5xl flex-col items-center px-4 py-20 text-center sm:px-6 sm:py-24">
          <div className="hero-content-from-left flex w-full max-w-5xl flex-col items-center opacity-0">
            <span className="inline-block rounded-full border border-white/50 bg-white/10 px-5 py-2 text-xs font-semibold uppercase tracking-wide text-white">
              Professional Services Communication Cloud
            </span>
            <h1 className="mx-auto mt-8 max-w-5xl text-[40px] font-bold leading-[1.12] tracking-tight text-white">
              Modernize Client Service Delivery with AI-Led Kloudvault
            </h1>
            <p className="mx-auto mt-6 max-w-2xl text-base leading-relaxed text-white/95 sm:text-lg">
              Unify support calls, advisory conversations, and follow-up workflows in Salesforce to improve service quality and long-term client trust.
            </p>
            <Link
            ...
            >
              Schedule a Professional Services Demo
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-center text-2xl font-bold text-slate-900 sm:text-3xl md:text-4xl">
            Measurable Outcomes for Service Teams
          </h2>
      ...
      <section className="bg-slate-50 py-14 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="mx-auto max-w-5xl text-center text-2xl font-bold leading-tight text-slate-900 sm:text-3xl md:text-[2.5rem] md:leading-tight">
            Enhance Every Client Interaction with Salesforce-Native Calling
          </h2>
          <p className="mx-auto mt-5 max-w-3xl text-center text-sm leading-relaxed text-slate-600 sm:text-base">
            Improve response quality, reduce communication delays, and maintain consistent client experiences with one connected calling workflow.
          </p>
          <div className="mt-14 grid items-start gap-10 lg:grid-cols-2 lg:gap-12">
            ...
            <div className="-mt-14 flex justify-center sm:-mt-16 lg:-mt-20 lg:justify-end">
              {enhanceImgIndex < ENHANCE_IMG_PATHS.length ? (
                <img
                  key={ENHANCE_IMG_PATHS[enhanceImgIndex]}
                  src={encodeURI(ENHANCE_IMG_PATHS[enhanceImgIndex])}
                  alt="Enhance the client journey with AI-driven Salesforce calling"
                  className="hero-content-from-right h-auto w-full max-w-xl rounded-2xl object-contain"
                  loading="lazy"
                  decoding="async"
                  onError={() => setEnhanceImgIndex((i) => i + 1)}
                />
              ) : (
                <div className="flex min-h-[200px] w-full max-w-xl flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-slate-300 bg-white px-4 py-8 text-center text-sm text-slate-600">
                  ...
                  <p>
                    Add <span className="font-mono text-xs">public/Enhance.png</span>
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid items-start gap-10 lg:grid-cols-2 lg:gap-12">
            <div className="hero-content-from-left order-2 -mt-2 flex justify-center opacity-0 sm:-mt-3 lg:order-1 lg:-mt-4 lg:justify-start">
              {resolveImgIndex < RESOLVE_IMG_PATHS.length ? (
                <img
                  key={RESOLVE_IMG_PATHS[resolveImgIndex]}
                  src={encodeURI(RESOLVE_IMG_PATHS[resolveImgIndex])}
                  alt="Resolve client disputes and schedule appointments"
                  className="h-auto w-full max-w-xl rounded-2xl object-contain"
                  loading="lazy"
                  decoding="async"
                  onError={() => setResolveImgIndex((i) => i + 1)}
                />
              ) : (
                <div className="flex min-h-[200px] w-full max-w-xl flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center text-sm text-slate-600">
                  <LogoMark className="h-12 w-12 opacity-40" />
                  <p>
                    Add <span className="font-mono text-xs">public/Resolve.png</span>
                  </p>
                </div>
              )}
            </div>
      ...
      <section className="bg-white py-10 sm:py-12">
        <div className="mx-auto max-w-3xl px-4 sm:px-6">
          <div className="flex flex-col items-stretch justify-between gap-6 rounded-2xl bg-[#3B8DEB] px-6 py-8 shadow-md sm:flex-row sm:items-center sm:justify-between sm:px-8 sm:py-9">
            ...
            >
              Download the Service Case Study
            </Link>
          </div>
        </div>
      </section>
      ...
      >
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-center text-2xl font-bold text-slate-900 sm:text-3xl">
            AI-Led Kloudvault Built for Professional Services
          </h2>
      ...
      <section className="bg-white py-14 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-center text-2xl font-bold text-slate-900 sm:text-3xl">
            What Service Teams Say About Kloudvault
          </h2>
      ...
      >
        <div className="mx-auto max-w-[min(100%,1200px)] px-4 text-center">
          <div className="flex justify-center overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            ...
            >
              Faster Response. Better Collaboration. Stronger Client Confidence.
            </h2>
          </div>
